...
def index(request):
    ...
    query = 'SELECT AUTH_USER.id AS id, AUTH_USER.username, AUTH_USER.is_active, AUTH_GROUP.name \
            from AUTH_USER \
...
def edit(request, id):
    ...
    user_query = 'SELECT AUTH_USER.id AS id, AUTH_USER.username, AUTH_USER.first_name, AUTH_USER.last_name, AUTH_USER.email, AUTH_USER.is_active \
            FROM AUTH_USER WHERE AUTH_USER.ID = ' + str(id)

    group_query = 'SELECT AUTH_GROUP.id AS id, AUTH_GROUP.name FROM AUTH_GROUP'

    selectedgroup_query = 'SELECT AUTH_GROUP.id AS id, AUTH_GROUP.name \
                        FROM AUTH_USER \
                        LEFT JOIN AUTH_USER_GROUPS ON AUTH_USER.id = AUTH_USER_GROUPS.user_id \
                        LEFT JOIN AUTH_GROUP ON AUTH_GROUP.id = AUTH_USER_GROUPS.group_id \
                        WHERE AUTH_USER.ID = ' + str(id)
    ...
    cursor.execute(selectedgroup_query)
    selectedgroups = cursor.fetchall()

    context = {
        'user': res[0],
        'groups': groups,
        'selectedgroups': selectedgroups,  # Rename to selectedgroups to avoid confusion
    }
...
def update(request, id):
    if request.method == 'POST':
        ...
        is_active = request.POST.get('userStatus', False)  # Adjusted to match the name attribute in the HTML form
        group_ids = request.POST.getlist('userGroup')
